...
function collectPageIds(html) {
  const ids = new Map();
  const pattern = /\bdata-page-id\s*=\s*(["'])(.*?)\1/g;
  let match;
  while ((match = pattern.exec(html))) {
    const id = match[2].trim();
    if (!ids.has(id)) ids.set(id, []);
...
function validateAnnotationShape(data) {
  ...
  for (const [index, annotation] of data.annotations.entries()) {
    ...
    for (const field of ["id", "pageId", "page", "category", "title", "target", "context", "status", "createdAt", "updatedAt"]) {
    ...
    if (annotation.pageId && !/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(annotation.pageId)) {
      errors.push(`${prefix}.pageId must use kebab-case`);
    }
    if (annotation.category && !categories.has(annotation.category)) errors.push(`${prefix}.category is invalid`);
    if (annotation.status && !statuses.has(annotation.status)) errors.push(`${prefix}.status is invalid`);
    if (!Array.isArray(annotation.rules) || annotation.rules.some((rule) => typeof rule !== "string" || !rule.trim())) {
      errors.push(`${prefix}.rules must contain non-empty strings`);
    }
    if (annotation.target && !annotation.target.annotationId && !annotation.target.selector) {
    ...
    for (const field of ["ruleId", "triggerCondition", "expectedOutcome", "acceptanceCriteria"]) {
      if (!annotation.context || !String(annotation.context[field] || "").trim()) {
        errors.push(`${prefix}.context.${field} is required`);
      }
    }
  }
  return errors;
...
function main() {
  ...
  const pageMap = collectPageIds(html);
  const htmlIdMap = collectHtmlIds(html);
  ...
  for (const [id, positions] of pageMap.entries()) {
    if (positions.length > 1) errors.push(`duplicate data-page-id in html: ${id}`);
  }

  for (const [id, positions] of htmlIdMap.entries()) {
    if (positions.length > 1) errors.push(`duplicate id in html: ${id}`);
  }

  for (const annotation of annotations.annotations || []) {
    if (!pageMap.has(annotation.pageId)) {
      errors.push(`${annotation.id}: page not found: ${annotation.pageId}`);
    }
    ...
    if (annotationId) {
      ...
      const htmlId = extractIdSelector(target.selector);
      if (htmlId) {
        const matches = htmlIdMap.get(htmlId) || [];
        if (matches.length === 0) errors.push(`${annotation.id}: target not found: #${htmlId}`);
        if (matches.length > 1) errors.push(`${annotation.id}: target is not unique: #${htmlId}`);
      } else {
