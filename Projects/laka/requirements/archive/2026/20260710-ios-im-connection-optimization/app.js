...
function renderWishGift() {
  ...
  const titleEl = document.getElementById('wgeTitle');
  const subEl = document.getElementById('wgeSub');
  ...
  setTimeout(() => {
    ...
    if (subEl) subEl.textContent = gift.sub;
  ...
  // 更新入口主题色（通过 CSS 变量或直接 style）
  entry.style.borderColor = gift.color + '40';
}
...
function handleWishGiftClick() {
  ...
  if (toast) {
    toast.textContent = `🎁 ${gift.title} — Opening...`;
    toast.classList.add('show');
  ...
  statusLog.innerHTML =
    '<b>💝 心愿礼物:</b><br/>' +
    `用户点击了 <b>${gift.title}</b> (${gift.id})。<br/>` +
    '进入对应心愿礼物赠送流程。';

  showToast(gift.title + ' selected!');
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  renderWishGift();
  // 启动自动轮播
  startWishGiftCarousel();

  // 箭头点击事件委托
  document.addEventListener('click', function(e) {
    const arrow = e.target.closest('.wge-arrow');
    if (arrow && !e.target.closest('.wish-gift-entry')) return; // 入口整体点击已由 onclick 处理
    // 箭头在入口内部，随整体点击处理，此处可额外做手动切换
  });
});
