(function () {
  ...
  function readPreferences() {
    try {
      ...
      if (typeof parsed.panelW === "number") state.panelW = parsed.panelW;
      if (typeof parsed.panelH === "number") state.panelH = parsed.panelH;
    } catch (error) {
  ...
  function applyPanelSize() {
    ...
    if (!panel) return;
    if (state.panelW) {
      panel.style.width = `${clamp(state.panelW, 280, Math.max(320, window.innerWidth - 24))}px`;
    }
    if (state.panelH) {
      panel.style.height = `${clamp(state.panelH, 300, Math.max(320, window.innerHeight - 24))}px`;
      panel.style.top = "8px";
  ...
  function syncPrototypeWorkspace() {
    ...
    if (!state.workspacePadding) {
      state.workspacePadding = {
        inline: document.body.style.getPropertyValue("padding-right"),
        priority: document.body.style.getPropertyPriority("padding-right"),
        computed: window.getComputedStyle(document.body).paddingRight,
      };
    }
    const panel = state.elements.panel;
    ...
    if (state.visible) {
      const basePadding = state.workspacePadding.computed || "0px";
      document.body.style.setProperty("padding-right", `calc(${basePadding} + ${reservedRight}px)`, state.workspacePadding.priority);
    } else if (state.workspacePadding.inline) {
      document.body.style.setProperty("padding-right", state.workspacePadding.inline, state.workspacePadding.priority);
    } else {
  ...
  // 单句模式：内容与状态标注统一为一句话，不区分标题与正文
  function sentenceText(annotation) {
    const sentence = String(annotation?.description || "").trim();
    return sentence || String(annotation?.title || annotation?.id || "").trim();
  }
  function hasSeparateBody(annotation) {
    const sentence = String(annotation?.description || "").trim();
    const title = String(annotation?.title || "").trim();
    return !!sentence && sentence !== title;
  }
  ...
  function ensureResizeGrip() {
    const panel = state.elements.panel;
    if (!panel) return;
    let grip = panel.querySelector(".paa-resize");
    if (!grip) {
      grip = document.createElement("button");
      grip.type = "button";
      grip.className = "paa-resize";
      grip.setAttribute("aria-label", "调整标注面板大小");
      grip.innerHTML = `<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M5 19h14" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"></path><path d="M9 15h10" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"></path><path d="M13 11h6" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"></path></svg>`;
      panel.appendChild(grip);
    }
    const resizeHandler = (down) => {
      if (!down.isPrimary) return;
      down.preventDefault();
      try { grip.setPointerCapture(down.pointerId); } catch (e) {}
      const startW = panel.getBoundingClientRect().width;
      const startH = panel.getBoundingClientRect().height;
      const startX = down.clientX;
      const startY = down.clientY;
      const onMove = (move) => {
        const w = clamp(startW - (move.clientX - startX), 280, Math.max(320, window.innerWidth - 24));
        const h = clamp(startH + (move.clientY - startY), 300, Math.max(320, window.innerHeight - 24));
        panel.style.width = `${Math.round(w)}px`;
        panel.style.height = `${Math.round(h)}px`;
        panel.style.top = "8px";
        panel.style.bottom = "auto";
        state.panelW = Math.round(w);
        state.panelH = Math.round(h);
        syncPrototypeWorkspace();
      };
      const onUp = () => {
    ...
    grip.onpointerdown = resizeHandler;
  ...
  function renderPreview() {
    ...
    const sentence = sentenceText(annotation);
    const separateBody = hasSeparateBody(annotation);
    const description = separateBody
    ...
    card.innerHTML = `
      <div class="paa-preview-header">
        <div class="paa-preview-heading">
          <span class="paa-preview-kicker">${annotationKind(annotation) === "state" ? "边界标注" : "内容标注"} · ${escapeHtml(CATEGORY_LABELS[annotationCategory(annotation)])}</span>
          <h3 class="paa-preview-title">${escapeHtml(sentence)}</h3>
        </div>
  ...
  function renderAgentWorkspace(annotation) {
    const kindLabel = annotationKind(annotation) === "state" ? "边界标注" : "内容标注";
  ...
  function renderMarkers() {
    ...
    markerAnnotations().forEach((annotation) => {
      ...
      marker.addEventListener("click", (event) => {
        event.preventDefault();
        event.stopPropagation();
        focusAnnotation(annotation.id, { scrollPanel: true, scrollTarget: false, showPreview: false });
      });
  ...
  function renderPanel() {
    ...
    panel.innerHTML = `
      ...
      <div class="paa-kind-tabs" role="tablist" aria-label="标注类型">
        ...
        <button class="paa-kind-tab paa-category-state${state.annotationKind === "state" ? " is-active" : ""}" type="button" role="tab" aria-selected="${state.annotationKind === "state"}" data-paa-kind="state"><span>边界标注</span><b>${state.doc.annotations.filter((item) => item.status !== "deprecated" && annotationKind(item) === "state").length}</b></button>
      </div>
  ...
  function renderPanelItem(annotation) {
    ...
    const rules = Array.isArray(annotation.rules) ? annotation.rules : [];
    const sentence = sentenceText(annotation);
    const description = hasSeparateBody(annotation)
      ...
      <article class="paa-item ${categoryClass(annotation)}${isActive ? " is-active" : ""}" data-paa-item="${escapeHtml(annotation.id)}" data-paa-category="${category}">
        <button class="paa-item-summary" type="button" data-paa-toggle="${escapeHtml(annotation.id)}" aria-expanded="${isActive}">
          ...
          <span class="paa-summary-copy">
            <span class="paa-item-title">${escapeHtml(sentence)}</span>
        ...
        ${isActive ? `
          ...
          <div class="paa-actions">
            ...
            <button class="paa-button is-danger${state.pendingDeleteId === annotation.id ? " is-pending" : ""}" type="button" data-paa-delete="${escapeHtml(annotation.id)}">${state.pendingDeleteId === annotation.id ? "确认删除？" : "删除"}</button>` : ""}
          </div>
        ` : ""}
      </article>`;
  }
  ...
  function renderEditForm(annotation, isActive) {
    ...
    const hiddenFields = (name, value) => `<input type="hidden" name="${name}" value="${escapeHtml(value || "")}">`;
    return `
      <article class="paa-item ${categoryClass(annotation)} paa-item-editing${isActive ? " is-active" : ""}" data-paa-item="${escapeHtml(annotation.id)}" data-paa-category="${annotationCategory(annotation)}">
        <form class="paa-form" data-paa-form="${escapeHtml(annotation.id)}">
          ${hiddenFields("pageId", annotation.pageId)}
          ${hiddenFields("page", annotation.page)}
          ${hiddenFields("ruleId", context.ruleId)}
          ${hiddenFields("status", annotation.status || "active")}
          ${hiddenFields("rules", Array.isArray(annotation.rules) ? annotation.rules.join("\n") : "")}
          ${hiddenFields("triggerCondition", context.triggerCondition)}
          ${hiddenFields("judgmentLogic", context.judgmentLogic)}
          ${hiddenFields("expectedOutcome", context.expectedOutcome)}
          ${hiddenFields("acceptanceCriteria", context.acceptanceCriteria)}
          <div class="paa-form-row paa-kind-pick" role="radiogroup" aria-label="标注类型">
            <label class="paa-kind-option${annotationCategory(annotation) === "state" ? "" : annotationCategory(annotation) === "edge" ? "" : " is-checked"} paa-kind-content"><input type="radio" name="category" value="page-flow"${annotationCategory(annotation) === "state" ? "" : annotationCategory(annotation) === "edge" ? "" : " checked"}>内容标注</label>
            <label class="paa-kind-option${annotationCategory(annotation) === "state" ? " is-checked" : ""} paa-kind-state"><input type="radio" name="category" value="state"${annotationCategory(annotation) === "state" ? " checked" : ""}>边界标注</label>
            <label class="paa-kind-option${annotationCategory(annotation) === "edge" ? " is-checked" : ""} paa-kind-edge"><input type="radio" name="category" value="edge"${annotationCategory(annotation) === "edge" ? " checked" : ""}>边界情况</label>
          </div>
          <textarea class="paa-textarea paa-sentence-input" name="description" rows="2" required placeholder="一句话描述">${escapeHtml(sentenceText(annotation))}</textarea>
  ...
  function saveForm(form) {
    ...
    const singleSentence = String(data.get("description") || "").trim() || String(data.get("title") || annotation.title || "").trim();
    annotation.title = singleSentence;
    ...
    annotation.description = singleSentence;
    annotation.rules = String(data.get("rules") || "").split("\n").map((item) => item.trim()).filter(Boolean);
    annotation.status = String(data.get("status") || annotation.status || "active");
    // 保证服务端必填字段非空，避免保存 400 导致标注无法落盘
    annotation.context = Object.assign({}, annotation.context || {}, {
      ruleId: String(data.get("ruleId") || annotation.context?.ruleId || "TBD").trim(),
      triggerCondition: String(data.get("triggerCondition") || annotation.context?.triggerCondition || "待补充").trim(),
      expectedOutcome: String(data.get("expectedOutcome") || annotation.context?.expectedOutcome || "待补充").trim(),
      acceptanceCriteria: String(data.get("acceptanceCriteria") || annotation.context?.acceptanceCriteria || "待补充").trim(),
    });
  ...
  function startPopupEdit(id) {
    ...
    render();
    window.setTimeout(() => state.elements.preview?.querySelector('textarea[name="description"]')?.focus(), 0);
  }
  ...
  function deleteAnnotation(id) {
    ...
    const annotation = state.doc.annotations.find((item) => item.id === id);
    if (!annotation) return;
    // 两步内联确认：第一次点击进入待确认，再次点击真正删除（不使用会被环境屏蔽的 window.confirm）
  ...
  const BUSINESS_CONTROL_SELECTOR = [
    "button", "a", "input", "select", "textarea", '[role="button"]',
    ".help-icon", ".icon-button", ".icon-pair span", ".asset", ".asset-shape",
    ".plan", ".privilege-card", ".tile-icon", ".identity-tile", ".benefit-orb",
    ".sheet-mark", ".benefit-mark", ".level-tag", ".chip", ".status-pill",
    ".filter-tabs span", ".vip-tier-strip span", ".toggle", ".recipient-row button",
    ".identity-tile i", ".gift-mini div", ".discount-mini div", ".source-table span"
  ].join(", ");
  ...
  function candidateFromEvent(event) {
    ...
    // 具体业务控件优先：icon/按钮/卡片/套餐/权益卡等，命中即锚定该控件
    const business = element.closest(BUSINESS_CONTROL_SELECTOR);
    if (business && !isRuntimeElement(business)) return business;
  ...
  function addAnnotationAt(candidate) {
    ...
    const sentence = suggestedTitle(candidate);
    const annotation = {
    ...
    stopSelection();
    render();
    window.setTimeout(() => state.elements.panel.querySelector(`[data-paa-form="${cssEscape(annotation.id)}"] textarea[name="description"]`)?.focus(), 0);
  }
