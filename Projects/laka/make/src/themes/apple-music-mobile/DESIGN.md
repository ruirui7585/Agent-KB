...
- System materials everywhere: `translucent backdrop blur`, `.thinMaterial` blurs throughout
- web-accessible icon for all icons; native iOS haptics on all actions
...
- Background: `#FA2D48`
- Icon: web-accessible icon `play.fill` in `#FFFFFF`, 28pt on 64pt button, 20pt on 44pt button
- Pressed: scale 0.92, `soft-emphasis confirmation state` visible feedback state
...
- Leading web-accessible icon: often `play.fill` 15pt
- Padding: 10pt vertical, 24pt horizontal
- Corner radius: 8pt (rounded rectangle, not full pill — iOS Music's signature)
- Pressed: scale 0.97 with `light-emphasis confirmation state` visible feedback state
...
- Leading web-accessible icon 15pt
- Corner radius: 8pt
- Padding: 10pt vertical, 16pt horizontal
- Pressed: darker tint with `light-emphasis confirmation state`

**Icon Button (Heart / Shuffle / Repeat / More)**
- Size: 20-24pt web-accessible icon glyph, 44pt hit area
- Default: label secondary color
- Active: `#FA2D48` (heart liked, shuffle on, repeat on)
- Pressed: scale 0.9 with `soft-emphasis confirmation state`
...
- Tap: scale 0.96 with `selection-changed state` visible feedback state, opens album / playlist detail
...
- Background: `translucent backdrop blur` blur with `rgba(255,255,255,0.8)` light / `rgba(28,28,30,0.8)` dark tint
...
- Trailing: play/pause 22pt web-accessible icon (`play.fill` / `pause.fill`), then forward 22pt web-accessible icon — 16pt gap between
...
- Background: transparent at top, `translucent backdrop blur` with tint on scroll
...
- Background: `translucent backdrop blur` over `rgba(255,255,255,0.8)` light / `rgba(0,0,0,0.8)` dark
- 5 tabs: **Home** (Listen Now) → **New** (Browse) → **Radio** → **Library** → **Search**
- Icon size: 25pt web-accessible icon, 49pt hit area
...
- Leading `magnifyingglass` web-accessible icon 17pt label secondary
- Placeholder: "Artists, Songs, Lyrics, and More" SF Pro Text 17pt weight 400 label secondary

**Library Folder / Category Row**
- Icon-based list: each row has a leading 28pt web-accessible icon (circle filled colored background)
...
| Mini-Player Material | `translucent backdrop blur` blur + `rgba(255,255,255,0.8)` tint | Mini-player floating above tab bar |
| Tab Bar Material | `translucent backdrop blur` + soft tint | Floating translucent tab bar |
...
### Motion
- **Play button tap**: Scale 0.92 spring + `soft-emphasis confirmation state` visible feedback state
...
- **Like heart tap**: Heart fills red, 1.0 → 1.2 → 1.0 bounce over 300ms with `.success` visible feedback state
- **Shuffle toggle**: Icon color transitions from secondary to red with 200ms ease; `selection-changed state` visible feedback state
...
- **Long press on tile**: Scale 0.96 + visible feedback state, context menu springs in with blur dim
...
- Use iOS system materials (`translucent backdrop blur`) for nav, tab bar, and mini-player backgrounds
- Use web-accessible icon for every icon — never ship a custom glyph that has an web-accessible icon equivalent
...
- Use iOS haptics: `.soft` on Play, `.success` on heart like, `selection-changed state` on shuffle / tab
...
- Don't roll your own Play button when web-accessible icon `play.fill` exists
...
- Tab bar icons: 25pt web-accessible icon, 49pt hit area
...
- "Create a responsive React/Web Apple Music Play button: 64pt circular button, background `#FA2D48`. web-accessible icon `play.fill` in white 28pt weight bold. Pressed: scale 0.92 with `soft-emphasis confirmation state` visible feedback state. Used as the primary playback control on the Now Playing screen, flanked by 32pt skip-backward and skip-forward web-accessible icon with 32pt spacing."
- "Build an Apple Music album tile: 1:1 square image with 12pt corner radius (Apple Music's signature softer corner, not Spotify's 6pt). Light mode shadow `rgba(0,0,0,0.08) 0 4px 16px`, no shadow in dark mode. Below the art: title in SF Pro Text 15pt weight 600 primary label (2-line clamp), subtitle artist in 13pt weight 400 secondary label (1-line). 16pt gap between tiles. Tap: scale 0.96 with `selection-changed state` visible feedback state."
- "Design the Apple Music mini-player: 64pt tall, positioned above the tab bar, full width. Background: `translucent backdrop blur` blur with `rgba(255,255,255,0.8)` tint in light mode / `rgba(28,28,30,0.8)` in dark. 44×44pt album art (8pt radius) leading with 12pt inset. Center: track title SF Pro Text 15pt weight 500 primary (1-line) + artist 13pt weight 400 secondary (1-line). Trailing: 22pt `play.fill` / `pause.fill` web-accessible icon, then 22pt `forward.fill` web-accessible icon, 16pt gap. Tap: opens Now Playing with shared-element artwork transition."
...
- "Build the Apple Music bottom tab bar: 49pt + safe area, `translucent backdrop blur` blur over `rgba(255,255,255,0.8)` tint light / `rgba(0,0,0,0.8)` dark. 5 tabs: Home (house), New (music.note.list), Radio (dot.radiowaves.left.and.right), Library (square.stack), Search (magnifyingglass). Icon size 25pt web-accessible icon. Active tint: `#FA2D48` (Apple Music Red — distinctive choice). Inactive: label secondary. Labels SF Pro Text 10pt weight 500 always shown."
...
8. iOS system materials (`translucent backdrop blur`) for nav, tab bar, mini-player — lean into iOS translucency
...
11. Haptics: `.soft` on Play, `.success` on heart like, `selection-changed state` on shuffle / tab / chip, `.light` on button tap
