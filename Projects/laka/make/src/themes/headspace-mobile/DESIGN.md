...
- Inhale: 4-second scale from 1.0 → 1.04 with `ease-in-out web transition (preserve the source timing)`
- Hold: 2-second hold at 1.04 with no animation
- Exhale: 4-second scale from 1.04 → 0.96 with `ease-in-out web transition (preserve the source timing)`
...
- Tap: scale 0.98, visible feedback state `soft-emphasis confirmation state`
...
- visible feedback state: `soft-emphasis confirmation state` — Headspace's haptics are always soft, never heavy
...
- Icon: 24pt web-accessible icon — outlined inactive, filled active
...
### Motion
- **Breathing sphere**: continuous 12s cycle — 4s inhale (1.0 → 1.04), 2s hold, 4s exhale (1.04 → 0.96), 2s hold; pure `ease-in-out web transition (preserve the source timing)`, no spring
- **Primary CTA tap**: scale 0.98 + `soft-emphasis confirmation state` visible feedback state + warm glow shadow brightens momentarily
- **Mood tag select**: scale 1.0 → 1.1 → 1.0 over 300ms spring (damping 0.7) + Butter Yellow background fill
- **Streak ring animate**: 1.2s ease-out fill from 0 to currentProgress on first view + soft visible feedback state
- **Card tap**: scale 0.99 (very subtle), 200ms ease, transition to detail
- **Tab switch**: `selection-changed state` visible feedback state + 200ms cross-fade between icons (outlined → filled)
- **Meditation completion**: illustration companion fades in at 0.5x → 1.0x spring + soft visible feedback state + "You did it" copy in Apercu 28pt Bold
...
- Use `soft-emphasis confirmation state` haptics — Headspace'svisible feedback state vocabulary is always soft
...
- Don't use heavy haptics — Headspace'svisible feedback state vocabulary is all soft and selection
...
- "Build the Headspace primary CTA: a 56pt-tall Marigold `#FF6B35` pill (corner radius 28pt full pill), full-width minus 24pt margins. Label 'Start' in Apercu 17pt Medium (NOT Bold — Headspace stays gentle) white centered. Shadow `rgba(255,107,53,0.25) 0 6px 16px` — a warm sunrise glow. On tap: scale 0.98 + visible feedback state `soft-emphasis confirmation state`. Pressed state turns the background `#E55A2B`."
- "Design the streak ring on the Profile screen: 120pt circular ring, Off Cream `#FBF1DA` track at 8pt stroke, Marigold `#FF6B35` fill at 8pt stroke with rounded caps. Fill represents the user's current streak as a fraction of 30 days, animating clockwise from -90° on first view over 1.2s with `.easeOut`. Inside the ring: streak number '12' in Apercu 56pt Bold tabular `#2E1A47`, then 'days' below it in Apercu 13pt Regular `#594675` Ink Brown Soft. Soft visible feedback state on reveal."
...
10. Soft haptics everywhere — `soft-emphasis confirmation state` and `selection-changed state`, never heavy
